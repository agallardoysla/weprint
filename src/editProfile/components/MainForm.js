...
export const MainForm = ({data}) => {
  ...
  const handleDatePick = (val, selectedDate, previousDate) => {
    setShowDatePicker(Platform.OS === 'ios');
    console.log('prevdate');
    console.log(previousDate);

    setDate(selectedDate ? selectedDate : previousDate);
    ...
    <View style={{width: '90%', marginVertical: 25}}>
      ...
      {showDatePicker && (
        <DateTimePicker
          ...
          is24Hour={true}
          display="spinner"
          onChange={(val, selectedDate) =>
            handleDatePick(val, selectedDate, date)
          }
        />
      )}
      <TextInput
