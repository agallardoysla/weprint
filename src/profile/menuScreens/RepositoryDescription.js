...
import {ScrollView, SafeAreaView, View, Text, StyleSheet, Image, TextInput, Modal, Alert} from 'react-native';
    ...
    return (
          <Container footer={false}>
            <View style={{backgroundColor: colores.fondoScreen, height: '100%'}}>
                            ...
                            <View style={{width: '100%', flexDirection: 'row', justifyContent: 'center', paddingVertical: 10}}>
                                <TouchableOpacity onPress={() => setShowListImage(true)}>
                                    <View style={{padding: 20, backgroundColor: colores.button, width: 190, borderRadius: 50, margin: 5}}>
                                        <Text style={{...estiloDeLetra.negrita, color: colores.blanco, textAlign:'center',fontSize: RFPercentage(1.5)}}>CARGAR FOTOS</Text>
                                    </View>
                                </TouchableOpacity>
                                <TouchableOpacity>
                                    <View style={{padding: 20, backgroundColor: colores.button, width: 190, borderRadius: 50, margin: 5}}>
                                        <Text style={{...estiloDeLetra.negrita, color: colores.blanco, textAlign:'center', fontSize: RFPercentage(1.5)}}>COMPARTIR</Text>
                                    </View>  
                                </TouchableOpacity>
                            </View>
                        ) : (
                ...
                <ScrollView>
                    <View style={{width: '95%', alignSelf: 'center', flexDirection: 'row', justifyContent: 'flex-start', flexWrap: 'wrap'}}>    
                        {
                            images && images.map((image, index) => <ImageRepository key={index} imageLink={image.base64} onPressFunction={() => handleDeleteImage(index)}/> )
                        }
                    </View>
                </ScrollView>
                <Modal visible={showListImage} hardwareAccelerated={true} animationType="slide" >
                    <SelectionListImage minQuantity={1} onResponse={(images) => {setShowListImage(false); setImages(images); setSendImages(true)}} 
